...
export function meta({ data }: Route.MetaArgs) {
	if (!data?.email) {
		return [
			...
			{ name: "description", content: "查看您在Smail临时邮箱中收到的邮件详情。" },
		...
		<div className="h-screen bg-gray-50 flex flex-col">
			<header className="bg-white border-b px-3 sm:px-4 py-3 shrink-0">
				<div className="w-full flex items-center justify-between">
					...
					{navigation.state === "loading" && <Loader2 className="w-3 sm:w-4 h-3 sm:h-4 animate-spin" />}
				</div>
			</header>
			...
			<div className="bg-white border-b px-3 sm:px-4 py-3 shrink-0">
				<div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
					<div className="flex-1 min-w-0">
						<h1 className="text-base sm:text-lg font-semibold text-gray-900 mb-2 break-words">{email.subject || "(无主题)"}</h1>
					...
					<div className="flex items-center gap-2 flex-shrink-0">
						<Badge variant={email.isRead ? "secondary" : "default"} className="text-xs">{email.isRead ? "已读" : "未读"}</Badge>
						<span className="text-xs text-gray-500">{formatFileSize(email.size)}</span>
					</div>
				</div>
				...
				{attachments.length > 0 && (
					<div className="mt-3 pt-3 border-t">
						<div className="flex items-center gap-2 mb-2">
							...
							<span className="text-xs sm:text-sm font-medium">附件 ({attachments.length})</span>
						</div>
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
							{attachments.map((attachment) => (
								<div key={attachment.id} className="flex items-center gap-2 px-2 py-1.5 bg-gray-50 border rounded text-xs">
									...
									{attachment.uploadStatus === "uploaded" ? (
										<a
											...
											className="inline-flex items-center justify-center h-6 w-6 p-0 rounded-md transition-colors hover:bg-accent hover:text-accent-foreground flex-shrink-0"
											title="下载附件"
										>
										...
										<span className="text-xs text-gray-400 flex-shrink-0">
											{attachment.uploadStatus === "pending" ? "处理中" : "失败"}
										</span>
									)}
								</div>
							))}
